...
public class RepositorioInquilino
{
    ...
    public IList<Inquilino> GetInquilinos()
    {
        ...
        {
            var sql =
                @$"SELECT {nameof(Inquilino.InquilinoID)}, 
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        inquilinos.Add(
                            new Inquilino
                            {
                                InquilinoID = reader.GetInt32(
                                    reader.GetOrdinal(nameof(Inquilino.InquilinoID))
                                ),
    ...
    public async Task<bool> ActualizarInquilino(Inquilino inquilino)
    {
        using (MySqlConnection connection = new MySqlConnection(ConnectionString))
        {
            var sql =
                @$"UPDATE inquilinos SET {nameof(Inquilino.Nombre)} = @Nombre, 
    ...
    public async Task<IList<Inquilino>> BuscarEnVivo(string term)
    {
        ...
        {
            string sql =
                @"SELECT InquilinoID, Nombre, Apellido, Dni, Telefono, Email, Estado
                       FROM inquilinos
    ...
    public Inquilino ObtenerInquilinoPorId(int inquilinoId)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection(ConnectionString))
        {
            var sql =
                ...
                + $"{nameof(Inquilino.Email)}, "
                + $"{nameof(Inquilino.Estado)} "
                + $"FROM inquilinos WHERE {nameof(Inquilino.InquilinoID)} = @InquilinoID;";
